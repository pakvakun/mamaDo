...
getAsyncData = async () => {
    ...
    fetchFilterInfo = () => {
        ...
        axios({
            ...
            params: {
                search: arrForSearch.stringForSearch,
                age_min: arrForSearch.age_min,
                age_max: arrForSearch.age_max,
                ages: arrForSearch.ages,
                distance: arrForSearch.metroStat && arrForSearch.metroStat.besideMe,
                metro_IDs: arrForSearch.metroStat && arrForSearch.metroStat.besideMetro,
                date_min: arrForSearch.date_min,
                date_max: arrForSearch.date_max,
                time_min: arrForSearch.time_min,
                time_max: arrForSearch.time_max,
                categories: arrForSearch.categories,
                type: arrForSearch.type,
                offer_types: arrForSearch.badge
            }
        }).then( res => {
            this.setState({companyEvents: res.data.data})
    ...
    getEvents = () => {
        axios({
            ...
            this.setState({companyEvents: response.data.data})
        })
    }
    componentDidMount(){
        this.getEvents()
            ...
            <View style={styles.container}>
                    ...
                    >
                        {
                            this.state.companyEvents && 
                                this.state.companyEvents.map((item, index) => (
                                    <Marker key={index} description={item.text} title={item.model.name} coordinate={{latitude: item.lat, longitude: item.long}}>
                                        <IconMarker />
                                    </Marker>
                                ))
                        }
                ...
                <Modal isVisible={this.state.isFilterVisible} style={styles.filter}>
                            ...
                            <View style={{flexDirection: 'row'}}>
                                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                                    ...
                                    <View style={{marginLeft: 7, backgroundColor: '#E94C89', borderRadius: 5, paddingLeft: 10, paddingRight: 10, paddingTop: 4, paddingBottom: 4}}>
                                        <Text style={{fontSize: 14, fontFamily: 'SF Compact Rounded', fontWeight: '700', color: '#FFF'}}>+2</Text>
                                    </View>
                                </View>
                    ...
                    <View style={{position: 'absolute', bottom: 0, width: '100%', height: 100, backgroundColor: '#FFF', paddingTop: 15, paddingLeft: 24, paddingRight: 24, shadowColor: '#455B63', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.1, shadowRadius: 12}}>
                        <TouchableOpacity style={{backgroundColor: '#E94C89', borderRadius: 10, paddingTop: 17, paddingBottom: 17}}>
                            <Text style={{textAlign: 'center', textTransform: 'uppercase', fontSize: 14, fontFamily: 'SF Pro Text', fontWeight: '600', color: '#FFF'}}>Показать {this.props.FilterEventsCount} занятий</Text>
                        </TouchableOpacity>
                    </View>
                </Modal>
