...
class BusinessEditEvent extends Component {
    ...
    getCurrentOfferType = currentOfferTypeId => {
        let currentOfferType = this.state.inputVidArr.filter(item => {
            return item.id == currentOfferTypeId
        })
        this.setState({inputVid: currentOfferType[0].name})
    }
    ...
    getEventInform = () => {
        let eventId = this.props.navigation.state.params.idEvent
        axios({
            method: 'GET',
            baseURL: 'https://mamado.elgrow.ru',
            ...
            let {event} = res.data
            this.getIdByCompanyName(false, false, event.company_id)
            this.getCurrentOfferType(event.offer_type_id)
            this.getCurrentEventType(event.type)
            this.getCurrentOrderType(event.order_type)
            this.handleChangeAge(`${event.age_min}/${event.age_max}`)
            this.getSchedule(event.event_schedules)
            this.getCategories(event.categories)
            this.getCurrentType(event.form)

            this.setState({
                inputName: event.name,
                inputPrice: `${event.price}`,
                inputLowPrice: `${event.discount}`,
                inputPhone: event.phone,
                inputSite: event.site,
                inputDesc: event.about,
                addresses: event.addresses,
                socialArr: event.soc,
                gallery_items: event.gallery_items,
                inputVidId: event.offer_type_id,
                date: this.state.valueTypeEvent === 0 ? event.event_time.date : '', 
                submitDate: this.state.valueTypeEvent === 0 ? event.event_time.date : '', 
                timeStart: this.state.valueTypeEvent === 0 ? moment(`${event.event_time.start}`, 'HH:mm:ss').format('hh:mm') : '',
                timeEnd: this.state.valueTypeEvent === 0 ? moment(`${event.event_time.end}`, 'HH:mm:ss').format('hh:mm') : '',
                notFormattedDateStart: this.state.valueTypeEvent === 2 ? event.event_period.start : false,
                notFormattedDateEnd: this.state.valueTypeEvent === 2 ? event.event_period.end : false,
    ...
    getIdByCompanyName = (name, index, id) => {
        ...
        if(index){
            ...
            this.setState({inputCompanyNameId: inputCompanyNameId[0].id, inputCompanyName: name, toggleCompanyName: !this.state.toggleCompanyName})
        }else{
            let inputCompanyNameId = this.state.inputCompanyNameArr.filter( (item, index) => {
                return item.id == id
            })
            this.setState({inputCompanyNameId: inputCompanyNameId[0].id, inputCompanyName: inputCompanyNameId[0].name})
        }
    }
    getIdByVidName = (name) => {
        //Еще один костыль на Picker (забрать вместо значения объект)
    ...
    getCurrentEventType = typeString => {
        let eventType = radio_type_event.filter(item => {
            return item.send == typeString
        })
        this.setState({
            inputTypeEvent: eventType[0].label,
            inputTypeEventSend: eventType[0].send,
            valueTypeEvent: eventType[0].value,

        })
    }
    getCurrentOrderType = orderTypeString => {
        let orderType = radio_type.filter(item => {
            return item.send == orderTypeString
        })
        orderType = orderType[0]

        this.setState({
            inputType: orderType.label,
            inputTypeSend: orderType.send,
            valueType: orderType.value
        })
    }
    getCurrentType = eventType => {
        if(eventType){
            let type = event_type.filter(item => {
                return item.send == eventType
            })
            console.log({eventType, type})
            type = type[0]
            this.setState({
                inputEventType: type.label,
                inputEventTypeSend: type.send,
                valueEventType: type.value
            })
        }
    }
    getSchedule = schedulesArray => {
        let normalizeScheduleForSend = schedulesArray.map( item => {
            return {
                date: item.date,
                time: item.event_schedule_times.map(elem => { return {start: elem.start, end: elem.end}}),
            }
        })
        let normalizeSchedule = schedulesArray.map( item => {
            return {
                date: moment(item.date, 'YYYY-MM-DD').format('LL'),
                time: item.event_schedule_times.map(elem => { return `${moment(elem.start, 'HH:mm:ss').format('HH:mm')} - ${moment(elem.end, 'HH:mm:ss').format('HH:mm')}`}),
            }
        })
        this.setState({sheduleArrForSend: normalizeScheduleForSend, sheduleArr: normalizeSchedule})
    }
    getCategories = categories => {
        checkedCategoeies = this.state.categoryArr.map(item => {
    ...
    submitEventInform = () => {
        let {
            ...
            socialArr && socialArr.forEach((element, index) => {
                data.append(`soc[${index}][url]`, element.url);
                data.append(`soc[${index}][key]`, element.key);
        ...
        axios({
            method: 'POST',
            baseURL: `https://mamado.elgrow.ru`,
            url: `/api/event/${this.props.navigation.state.params.idEvent}`,
            data: data,
            headers: {
                Authorization: this.state.token,
    ...
    render(){
        ...
        return (
            <View style={styles.container}>
                ...
                <Header
                    centerComponent={{ text: 'Событие', style: { color: '#FFF', fontFamily: 'SF Pro Text', fontSize: 17, fontWeight: '500' } }}
                    containerStyle={{
                        backgroundColor: '#556086',
                        borderBottomWidth: 0
                    }}
                    rightComponent={<View style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center'}}><TouchableOpacity onPress={this.isModalUnPublic} style={{right: 35}}><IconPublic/></TouchableOpacity><TouchableOpacity onPress={this.isModalDelete} style={{right: 10}}><IconTrash/></TouchableOpacity></View>}
                    leftComponent={<TouchableOpacity onPress={() => this.props.navigation.goBack()} style={{left: 20}}><IconBackWhite/></TouchableOpacity>}
                />
                ...
                <ScrollView contentContainerStyle={styles.scrollContainer}>
                        ...
                        <View style={{marginTop: 30}}>
                            <View style={{flexDirection: 'row', alignItems: 'center'}}>
                                <Text style={this.state.inputVid.length > 0  ? styles.labelActive : styles.labelNotActive}>Тип мероприятия</Text>
                                <View style={{marginLeft: 'auto'}}>
                                    <IconHelpInput/>
                                </View>
                            </View>
                        ...
                        <View style={{marginTop: 30}}>
                            ...
                            <View>
                                <TouchableOpacity onPress={this.state.inputType.length > 0 ? ()=>this.toggleModalEventType() : null } style={styles.startEndBlock}>
                                    ...
                                    {this.state.inputEventType && this.state.inputEventType.length ? <InputCheck style={{position: 'absolute', right: 20, top: 19}}/> : <InputPlus style={{position: 'absolute', right: 20, top: 19}}/>}
                                </TouchableOpacity>
                            </View>
                        </View>
                        <View style={{marginTop: 30}}>
                        ...
                        <TouchableOpacity onPress={()=>this.submitEventInform()} style={styles.createBlock}>
                            <Text style={styles.createBlockText}>Сохранить</Text>
                        </TouchableOpacity>
                    </ScrollView>
                </View>
                ...
                <Modal isVisible={this.state.isModalType} style={styles.modal}>
                    <View style={styles.modalMinContent}>
                        <View style={{height: 75, backgroundColor: '#556086', flexDirection: 'row', alignItems: 'center'}}>
                            <Text style={{color: '#FFF', fontFamily: 'SF Pro Text', fontSize: 16, fontWeight: '500', marginLeft: 25}}>Выберите тип заказа</Text>
...
export default BusinessEditEvent;
